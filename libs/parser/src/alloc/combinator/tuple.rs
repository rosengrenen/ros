...
macro_rules! tuple_trait_impl {
    ($($outputs:ident $parsers:ident),+) => {
        impl<'alloc, I, $($outputs),+, E, $($parsers),+, A> ParserAlloc<'alloc, I, A> for ($($parsers),+)
        where
            E: ParseError<I>,
            $($parsers: ParserAlloc<'alloc, I, A, Output = $outputs, Error = E>,)+
            A: Allocator
        {
            type Output = ($($outputs),+);
            ...
            fn parse_alloc(&self, input: I, alloc: &'alloc A) -> ParseResult<I, Self::Output, Self::Error> {
                ...
                $(
                    #[allow(non_snake_case)]
                    let (input, $outputs) = $parsers.parse_alloc(input, alloc)?;
                )+
