...
impl DefBreak {
    pub fn p<I: Input<Item = u8>, E: ParseError<I, A>, A: Allocator + Clone>(
        ...
        break_op.map(|_| Self).parse(input, alloc)
    }
}
...
impl DefBreakPoint {
    pub fn p<I: Input<Item = u8>, E: ParseError<I, A>, A: Allocator + Clone>(
        ...
        break_point_op.map(|_| DefBreakPoint).parse(input, alloc)
    }
}
...
impl DefContinue {
    pub fn p<I: Input<Item = u8>, E: ParseError<I, A>, A: Allocator + Clone>(
        ...
        continue_op.map(|_| Self).parse(input, alloc)
    }
}
...
impl DefNoop {
    pub fn p<I: Input<Item = u8>, E: ParseError<I, A>, A: Allocator + Clone>(
        ...
        noop_op.map(|_| Self).parse(input, alloc)
    }
}
...
impl<A: Allocator + Clone> EventObj<A> {
    pub fn p<I: Input<Item = u8>, E: ParseError<I, A>>(
        ...
        SuperName::p.map(Self).parse(input, alloc)
    }
}
